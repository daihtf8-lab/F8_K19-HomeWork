const renderTable = (headers, rows, className = null) => {
    ...
    for (const row of rows) {
        ...
        action.innerHTML = `
            <label for="popup-toggle" class="action-icon edit-btn" data-id="${row.id}" title="Edit">✏️</label>
            <span class="action-icon delete delete-btn" data-id="${row.id}" title="Delete">🗑️</span>
        `;
